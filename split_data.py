...
import numpy as np
# 静音 divide-by-zero 警告，避免 DataLoader worker 重复打印
...
from models.dataset_test import ATOM_LIST, CHIRALITY_LIST, BOND_LIST, BONDDIR_LIST
...
def get_classification_dataset(dataset: str,
                               ...
                               seed: int,
                               split_ratio:list):
    assert dataset in ['Tox21', 'ClinTox', 'AMES',
                       'SIDER', 'BBBP', 'BACE']
    ...
    def get_data(sub_data):
        ...
        for i in range(len(smiles)):
            ...
            for atom in mol.GetAtoms():
                ...
                if atomic_num in ATOM_LIST:
                    type_idx.append(ATOM_LIST.index(atomic_num))  # 0~118
                else:
                    type_idx.append(0)  # mask for uncommon elements

                tag = atom.GetChiralTag()
                # Map CHI_UNSPECIFIED/CW/CCW → 0/1/2; others → 0
                if tag in CHIRALITY_LIST[:3]:
                    chirality_idx.append(CHIRALITY_LIST.index(tag))
                else:
        ...
        return gs, ys, ms, task_weights,mogen_fp,maccs_fp,rdit_fp,X,A, gin_data_list
    ...
    train_gs, train_ls, train_masks, train_tw,morgan_fp_list_train,macc_train,ecfp_train,X_tain,A_train, gin_train= get_data(train)
    val_gs, val_ls, val_masks, val_tw,morgan_fp_list_val,macc_val,ecfp_val ,X_val,A_val, gin_val= get_data(val)
    test_gs, test_ls, test_masks, test_tw ,morgan_fp_list_test,macc_test,ecfp_test,X_test,A_test, gin_test= get_data(test)


    return train_gs, train_ls,train_tw, val_gs, val_ls,test_gs,test_ls,morgan_fp_list_train,\
        morgan_fp_list_val,morgan_fp_list_test,macc_train,macc_val,macc_test,ecfp_train,ecfp_val,ecfp_test,X_tain,X_val,X_test,A_train,A_val,A_test, gin_train, gin_val, gin_test
